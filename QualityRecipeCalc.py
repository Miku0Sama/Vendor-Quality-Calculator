...
def find_combinations(numbers=[1, 20], max_waste=2, flasks_or_gems=False):
    ...
    def is_usable(combo):
        ...
        for num, count in combo_count.items():
            if used_numbers[num] + count > numbers.count(num):  # Ensure it doesn't exceed the original count
                return False
        return True
    ...
    unused_numbers = [num for num in numbers if used_numbers[num] < numbers.count(num)]
...
output_combinations, unused_numbers = find_combinations(numbers, max_waste, flasks_or_gems)
print("Input:", numbers)
